...
def bootstrap_node(
    *,
    ...
    """Generate (or import) the node's three identity keys and write the
    ...
    Writes:
      - operator.toml (0600): server_sk / operator_sk / control_relay / admins
      - portal.toml (0640 root:ynh-portal): the low-privilege notice key
      - optionally a relay config (``write_relay``) that allowlists the three
        keys as writers on the local control plane.
    ...
    for name, sk in (("--server-sk", server_sk), ("--operator-sk", operator_sk), ("--notice-sk", notice_sk)):
        if not _is_hex64(sk):
