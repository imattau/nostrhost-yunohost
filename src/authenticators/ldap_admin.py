...
class Authenticator(BaseAuthenticator):  # type: ignore
    """Admin session-cookie management (moulinette's ``api: ldap_admin``
    credential check is retired along with moulinette itself -- see
    ``docs/LDAP-RETIREMENT.md``; sessions are established via the native
    Nostr admin API, not a password bind, so this class no longer overrides
