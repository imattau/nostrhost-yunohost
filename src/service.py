...
def service_add(
    ...
    logger.success(tr("service_added", service=name))
...
def service_remove(name):
    ...
    logger.success(tr("service_removed", service=name))
...
def service_start(names):
    ...
    for name in names:
        if _run_service_command("start", name):
            logger.success(tr("service_started", service=name))
        else:
            ...
            logger.debug(tr("service_already_started", service=name))
...
def service_stop(names):
    ...
    for name in names:
        if _run_service_command("stop", name):
            logger.success(tr("service_stopped", service=name))
        else:
            ...
            logger.debug(tr("service_already_stopped", service=name))
...
def service_reload(names):
    ...
    for name in names:
        if _run_service_command("reload", name):
            logger.success(tr("service_reloaded", service=name))
        else:
            if service_status(name)["status"] != "running":
                logs = _get_journalctl_logs(name, number=25)
                if Moulinette.interface.type != "api":
...
def service_restart(names):
    ...
    for name in names:
        if _run_service_command("restart", name):
            logger.success(tr("service_restarted", service=name))
        else:
            if service_status(name)["status"] != "running":
                logs = _get_journalctl_logs(name, number=25)
                if Moulinette.interface.type != "api":
...
def service_reload_or_restart(names, test_conf=True):
    ...
    for name in names:
        ...
        if _run_service_command("reload-or-restart", name):
            logger.success(tr("service_reloaded_or_restarted", service=name))
        else:
            if service_status(name)["status"] != "running":
                logs = _get_journalctl_logs(name, number=25)
                if Moulinette.interface.type != "api":
...
def service_enable(names):
    ...
    for name in names:
        if _run_service_command("enable", name):
            ...
            logger.success(tr("service_enabled", service=name))
        else:
...
def service_disable(names):
    ...
    for name in names:
        if _run_service_command("disable", name):
            ...
            logger.success(tr("service_disabled", service=name))
        else:
...
def _get_and_format_service_status(service, infos):
    ...
    if not description:
        ...
        if key_exists(translation_key):
            description = tr(translation_key)
        else:
...
def _run_service_command(action: str, service: str) -> bool:
    ...
    try:
        ...
        if p.returncode != 0:
            logger.warning(tr("service_cmd_exec_failed", command=cmd))
            return False

    except Exception as e:
        logger.warning(tr("unexpected_error", error=str(e)))
        return False
