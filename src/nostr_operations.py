...
from .nostrhost.nsites.operations import (  # noqa: E402 - nsite.* input models + handlers
    BlockPubkeyArgs,
    BlockSetArgs,
    CollectionDiscoverArgs,
    CollectionPlanArgs,
    CollectionPublishArgs,
    CollectionResolveArgs,
    CollectionValidateArgs,
    DiscoverArgs,
    DomainAttachArgs,
    DomainDetachArgs,
    BlossomDisableArgs,
    ...
    _safe_nsite_collection_discover,
    _safe_nsite_collection_plan,
    _safe_nsite_collection_publish,
    _safe_nsite_collection_resolve,
    _safe_nsite_collection_validate,
...
TOOLS: dict[str, ToolSpec] = {
    ...
    "nsite.discover": ToolSpec(
        ...
        scope=SCOPE_NSITES_READ,
        require_approval=False,
        input_model=DiscoverArgs,
        description="browse validated nsite manifests (15128/35128) found on the catalogue + lookup relays (read only, bounded, 5-min cache; refresh=1 forces a live scan)",
    ),
    ...
    "nsite.block.list": ToolSpec(
        name="nsite.block.list",
        handler=_safe_nsite_block_list,
        scope=SCOPE_NSITES_READ,
        require_approval=False,
        description="the operator's kind-10000 mute list (blocked npub pubkeys) from the control relay",
    ),
    "nsite.block.set": ToolSpec(
        name="nsite.block.set",
        handler=_safe_nsite_block_set,
        scope=SCOPE_NSITES_ADMIN,
        input_model=BlockSetArgs,
        risk=RISK_LOW,
        reversibility=REVERSIBLE,
        description="replace the operator's kind-10000 mute list with exactly these pubkeys (empty clears it)",
    ),
    "nsite.block.add": ToolSpec(
        name="nsite.block.add",
        handler=_safe_nsite_block_add,
        scope=SCOPE_NSITES_ADMIN,
        input_model=BlockPubkeyArgs,
        risk=RISK_LOW,
        reversibility=REVERSIBLE,
        description="add one npub pubkey to the operator's mute list (excludes its sites from discover)",
    ),
    "nsite.block.remove": ToolSpec(
        name="nsite.block.remove",
        handler=_safe_nsite_block_remove,
        scope=SCOPE_NSITES_ADMIN,
        input_model=BlockPubkeyArgs,
        risk=RISK_LOW,
        reversibility=REVERSIBLE,
        description="remove one npub pubkey from the operator's mute list",
    ),
    ...
    "nsite.collection.validate": ToolSpec(
        name="nsite.collection.validate",
        handler=_safe_nsite_collection_validate,
        scope=SCOPE_NSITES_READ,
        require_approval=False,
        input_model=CollectionValidateArgs,
        description="validate a candidate kind-30004 collection event (t=nsite, entries, signature); no network",
    ),
    "nsite.collection.get": ToolSpec(
        name="nsite.collection.get",
        handler=_safe_nsite_collection_resolve,
        scope=SCOPE_NSITES_READ,
        require_approval=False,
        input_model=CollectionResolveArgs,
        description="resolve one collection coordinate and its ordered live/pinned entries (read only, bounded)",
    ),
    "nsite.collection.discover": ToolSpec(
        name="nsite.collection.discover",
        handler=_safe_nsite_collection_discover,
        scope=SCOPE_NSITES_READ,
        require_approval=False,
        input_model=CollectionDiscoverArgs,
        description="browse validated kind-30004 t=nsite collections found on the catalogue + lookup relays (read only, bounded, 5-min cache; refresh=1 forces a live scan)",
    ),
    "nsite.collection.publish.plan": ToolSpec(
        name="nsite.collection.publish.plan",
        handler=_safe_nsite_collection_plan,
        scope=SCOPE_NSITES_READ,
        require_approval=False,
        input_model=CollectionPlanArgs,
        description="ordered entries + metadata to an unsigned kind-30004 collection + plan_sha256 (D7); nothing is signed or broadcast",
    ),
    "nsite.collection.publish": ToolSpec(
        name="nsite.collection.publish",
        handler=_safe_nsite_collection_publish,
        scope=SCOPE_NSITES_PUBLISH,
        input_model=CollectionPublishArgs,
        risk=RISK_MEDIUM,
        reversibility=REVERSIBLE,
        description="verify a signed kind-30004 collection (signature, plan digest, bounds), broadcast to relays, report per-relay results",
    ),
