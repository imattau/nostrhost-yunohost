"""List/preference projector daemon (nostr-listd) — WP4.

Subscribes to the NIP-51/NIP-65/NIP-78 kinds the host projects (mute list,
preferred/blocked relays, people sets, application data) and folds them into
the ``ListStore`` + family compatibility files. Runs on the shared
:class:`~nostr_projector.ProjectionRuntime`; rebuild/verify/shadow are wired
through ``bin/nostr-projector``.
"""

from __future__ import annotations

import asyncio
import logging
import threading
from pathlib import Path
from typing import Any, Callable

from .nostrhost.list_projection import LIST_KINDS, ListProjector, ListStore
from .nostrhost.list_specs import is_self_service_coordinate
from .nostr_projector import (
    DEFAULT_CURSOR_DIR,
    ProjectionRuntime,
    REGISTRY,
)
from .nostr_identity import (
    _configure_daemon_logging,
    _init_headless_yunohost,
    _operator_config,
    _require_bootstrapped,
)

logger = logging.getLogger("nostr-listd")


def _default_render(entry: Any) -> None:
    """Render a changed coordinate into its family compatibility file(s).

    Only operators' host lists produce compatibility files (trusted
    publishers → catalogue env, portal settings → portal files). User
    preferences need no rendered file: they are read on demand by the portal
    through :func:`nostrhost.list_projection.user_preferences`.
    """
    try:
        from .nostrhost import list_render
    except Exception:  # noqa: BLE001 - render layer is optional
        return
    list_render.render_entry(entry)


async def subscribe_loop(
    relay_url: str,
    *,
    store: ListStore,
    admin_pubkeys: tuple[str, ...] | list[str],
    on_change: Callable[[Any], None] | None = None,
    stop: asyncio.Event | None = None,
    cursor_dir: str | Path = DEFAULT_CURSOR_DIR,
) -> None:
    projector = ListProjector(
        store=store,
        admin_pubkeys=admin_pubkeys,
        validate_self_service=is_self_service_coordinate,
        on_change=on_change or _default_render,
        cursor_dir=cursor_dir,
    )
    REGISTRY.register(projector)
    runtime = ProjectionRuntime(
        relay_url,
        projector=projector,
        kinds=list(LIST_KINDS),
        limit=2000,
        stop=stop,
    )
    await runtime.run()


def rebuild(relay_url: str, *, admin_pubkeys: tuple[str, ...] | list[str]) -> dict[str, Any]:
    """Rebuild the list projection from the relay (WP4)."""
    from .nostr_projector import rebuild as _rebuild
    from .nostrhost.events import query_chain_events

    events: list[dict[str, Any]] = []
    for kind in LIST_KINDS:
        events.extend(query_chain_events(relay_url, kinds=(kind,), limit=500))
    projector = ListProjector(admin_pubkeys=admin_pubkeys)
    report = _rebuild(events, projector=projector)
    report["projection"] = projector._digest()
    return report


def run() -> None:
    """Entry point for bin/nostr-listd."""
    _configure_daemon_logging()
    _init_headless_yunohost()
    _require_bootstrapped()
    cfg = _operator_config()
    store = ListStore()
    stop = threading.Event()
    logger.info("list projector subscribing to %s", cfg.control_relay)
    try:
        asyncio.run(
            subscribe_loop(
                cfg.control_relay,
                store=store,
                admin_pubkeys=cfg.admins,
            )
        )
    except KeyboardInterrupt:
        pass
    finally:
        stop.set()
